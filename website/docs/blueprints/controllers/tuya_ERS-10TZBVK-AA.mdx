...
<ImportCard id='ERS-10TZBVK-AA' category='controllers' />
...
Hooks allow to easily create controller-based automations for interacting with lights, media players, covers and maybe more in the future. See the list of [Hooks available for this controller](/docs/blueprints/controllers/ERS-10TZBVK-AA#available-hooks) for additional details.
:::
...
/>
<Input
  name='Rotate left withpress [Only in Command Mode]'
  description='Action to run on rotate left withpress.'
  selector='action'
/>
<Input
  name='Rotate right withpress [Only in Command Mode]'
  description='Action to run on rotate right withpress.'
  selector='action'
/>
<Input
...
/>
<Input
  name='Remote double press [`Virtual` in Command Mode]'
  description='Action to run on double remote press.'
  selector='action'
  virtual
/>
<Input
  name='Remote long press'
...
/>
<Input
  name='Rotate left withpress - loop until stop [Only in Command Mode]'
  description='Loop the rotate left withpress action until the rotation is stopped.'
  selector='boolean'
/>
<Input
  name='Rotate left withpress - Maximum loop repeats [Only in Command Mode]'
  description='Maximum number of repeats for the custom action, when looping is enabled. Use it as a safety limit to prevent an endless loop in case the corresponding stop event is not received.'
  selector='number'
/>
<Input
  name='Rotate right withpress - loop until stop [Only in Command Mode]'
  description='Loop the rotate right withpress action until the rotation is stopped.'
  selector='boolean'
/>
<Input
  name='Rotate right withpress - Maximum loop repeats [Only in Command Mode]'
  description='Maximum number of repeats for the custom action, when looping is enabled. Use it as a safety limit to prevent an endless loop in case the corresponding stop event is not received.'
  selector='number'
/>
<Input
...
- **2025-02-28**: Initial release. ([@yarafie](https://github.com/yarafie)) :tada:
