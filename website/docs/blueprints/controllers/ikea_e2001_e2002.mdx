...
The helper is used to determine button release events when the controller is integrated with Zigbee2MQTT and ZHA, because of the actions mapping for the controller with these integrations. Natively, the controller doesn't allow to distinguish between different button release events for some of it's buttons, so the blueprint must store the previous clicked button.
...
- **2025-11-11**: Refactored - Added back Helpers to Fix Single/Double/Long Press/Release Logic for both hooks and custom actions. ([EPMatt-issue#949](https://github.com/EPMatt/awesome-ha-blueprints/issues/949)) ([@yarafie](https://github.com/yarafie))
- **2025-11-16**: Fixed ZHA Up Button Long/Release event triggers. ([@yarafie](https://github.com/yarafie))
