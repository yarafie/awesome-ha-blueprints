...
### Issues with ZHA specificaly for the E2002 and the device firing the on event ahead of the press/hold events

It has been reported that IKEA E2002 controller integrated with ZHA might fire the on event when a long press is done on the left/right arrow buttons, during interaction. This issue is not relative to the blueprint itself. The blueprint tries to account for this to a certain degree.
