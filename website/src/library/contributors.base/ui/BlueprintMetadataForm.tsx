...
import { isHttpsUrl } from '../services/deriveBlueprintMetadata'
...
interface Props {
  ...
  /**
   * For blueprint:update: assets already exist; disable asset pickers here.
   * For blueprint:new: assets may be selected.
   */
...
export default function BlueprintMetadataForm({
  ...
  // Locked: PDF only, renamed internally to <blueprint_id>.pdf
    ...
    <section className='container padding-vert--lg'>
      ...
      >
        ...
        <div style={{ display: 'grid', gap: 12 }}>
          ...
          <div>
            <div style={{ fontSize: 13, opacity: 0.8, marginBottom: 6 }}>
              librarians (locked order; unique): yarafie, logged-in user
            </div>
            <input
            ...
            />
            <div style={{ fontSize: 12, opacity: 0.7, marginTop: 6 }}>
              Read-only (always derived). Stored as array of objects
              (id/name/url).
            </div>
          </div>
            ...
            {assetsReadOnly ? (
              <div style={{ fontSize: 13, opacity: 0.8 }}>
                ...
                <div style={{ fontSize: 12, opacity: 0.65 }}>
                  Image upload is disabled for update flow.
                </div>
              </div>
            ) : (
              <>
          ...
          <div>
            ...
            {assetsReadOnly ? (
              <div style={{ fontSize: 13, opacity: 0.8 }}>
                Existing file(s):
                <ul>
                  {draft.manual_files.map((f) => (
                    <li key={f}>{f}</li>
                  ))}
                </ul>
                <div style={{ fontSize: 12, opacity: 0.65 }}>
                  PDF upload is disabled for update flow.
                </div>
              </div>
            ) : (
              <>
          ...
          <div>
            <div style={{ fontSize: 13, opacity: 0.8, marginBottom: 6 }}>
              external_references (optional; valid https links only)
            </div>
              ...
              return (
                ...
                >
                  <div style={{ display: 'grid', gap: 10 }}>
                    <input
                      placeholder='Label (e.g. Docs, Source, Blakadder)'
                      value={ref.label}
                      onChange={(e) =>
          ...
          {/* Hidden: supported_integrations (not required for automation MVP) */}
          <div style={{ fontSize: 12, opacity: 0.65 }}>
            supported_integrations: hidden (not required for Phase 3)
          </div>
        </div>
      </div>
    </section>
